...
public class MobileBasePage {
    ...
    protected void initPageElements() {
        PageFactory.initElements(new AppiumFieldDecorator(getAppiumDriver(), Duration.ofSeconds(5)), this);
    }
    ...
    protected void swipeRightOnScreen(int times){
        Dimension size = AppiumWrapper.getAppiumDriver().manage().window().getSize();
        int startX = (int) (size.width * 0.20);
        int endX = (int) (size.width * 0.8);
        int startY = size.height / 2;
        for(int i = 0; i < times; i++){
            new TouchAction(AppiumWrapper.getAppiumDriver()).press(PointOption.point(endX,startY)).moveTo(PointOption.point(startX,startY)).release().perform();
        }
    }
}
